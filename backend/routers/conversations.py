...
async def get_interaction_repository() -> InteractionRepository:
    from backend.main import app

    return InteractionRepository(app.mongodb)


@router.post("/get_session_id")
async def get_session_id_by_story(story_id: str):
...
async def send_user_message_and_update_db(user_message: UserMessage):
    interaction_repo = await get_interaction_repository()
